...
function DetailsPanel({ session, events, onClose, showClose = true }: { session: Session; events: SessionEvent[]; onClose: () => void; showClose?: boolean }) {
  ...
  return (
    <aside className="session-details" aria-label="Session details">
      ...
      <section className="details-section"><div className="details-section-heading"><h3>Run usage</h3><Badge variant="outline">{latestRun ? statusLabel(latestRun.status) : 'Not started'}</Badge></div><dl className="detail-list"><div><dt>Runs</dt><dd>{session.runs.length}</dd></div><div><dt>Active run</dt><dd>{session.activeRunId ? 'Live' : 'None'}</dd></div><div><dt>Events received</dt><dd>{events.length}</dd></div><div><dt>Mode</dt><dd>{session.mode === 'plan' ? 'Plan only' : 'Controlled execute'}</dd></div></dl></section>
